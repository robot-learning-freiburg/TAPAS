...
class VitEncoderModel(ViTExtractor):
    """
    Simple wrapper class to pass structured config and save as single torch model.
    """

    def __init__(
        self,
        config: VitFeatureModelConfig,
        device: torch.device,
        model: nn.Module | None = None,
    ):
        self.model_type = config.vision_net
        self.device = device
        if model is not None:
            self.model = model
        else:
            self.model = ViTExtractor.create_model(self.model_type)

        self.model = ViTExtractor.patch_vit_resolution(self.model, stride=config.stride)
        self.model.eval()
        self.model.to(self.device)
        self.stride = self.model.patch_embed.proj.stride
    ...
    def extract_descriptors(self, batch: torch.Tensor) -> torch.Tensor:
        return super().extract_descriptors(
            ...
            bin=self.config.bin,
            include_cls=self.config.include_cls,
            register_tokens=self.config.register_tokens,
        )
