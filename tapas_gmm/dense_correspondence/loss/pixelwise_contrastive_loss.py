...
class PixelwiseContrastiveLoss:
    ...
    def get_loss_matched_and_non_matched_with_l2(
        self,
        image_a_pred,
        image_b_pred,
        matches_a,
        ...
        match_loss =
          1/num_matches \\sum_{num_matches} ||descriptor_a - descriptor_b||_2^2
        non_match_loss =
          1/num_non_matches \\sum_{num_non_matches}
          max(0, M_margin - ||descriptor_a - descriptor_b||_2)^2
        ...
        :type image_b_pred:
        :param matches_a: torch.Variable(torch.LongTensor) has shape
        [num_matches,],  a (u,v) pair is mapped to (u,v)
        ---> image_width * v + u, this matches the shape of one dimension of
        image_a_pred
        :type matches_a: torch.Variable(torch.FloatTensor)
        :param matches_b: same as matches_a
        :type matches_b:
        :param non_matches_a: torch.Variable(torch.FloatTensor) has shape
        [num_non_matches,],  a (u,v) pair is mapped to (u,v)
        ---> image_width * v + u, this matches the shape of image_a_pred
        ...
        match_loss, _, _, match_descriptor_dist = PCL.match_loss(
            image_a_pred,
            image_b_pred,
            matches_a,
            matches_b,
            norm_by_descriptor_dim=self._config.norm_by_descriptor_dim,
        )
        ...
        if use_l2_pixel_loss:
            ...
            ) = self.non_match_loss_with_l2_pixel_norm(
                image_a_pred,
                image_b_pred,
                matches_b,
                non_matches_a,
            ...
            (
                ...
                num_hard_negatives,
                non_match_descriptor_dist,
            ) = self.non_match_loss_descriptor_only(
                image_a_pred,
                image_b_pred,
                non_matches_a,
                non_matches_b,
    ...
    def get_triplet_loss(
        ...
        \\sum_{triplets} ||D(I_a, u_a, I_b, u_{b,match})||_2^2 -
        ||D(I_a, u_a, I_b, u_{b,non-match)||_2^2 + alpha
        ...
        triplet_losses = (
            (matches_a_descriptors - matches_b_descriptors).pow(2)
            - (matches_a_descriptors - non_matches_b_descriptors).pow(2)
            + alpha
        )
    ...
    @staticmethod
    def match_loss(
        image_a_pred, image_b_pred, matches_a, matches_b, norm_by_descriptor_dim=False
    ):
        """
        ...
        matches_a_descriptors is torch.FloatTensor with shape
        torch.Shape([num_matches, descriptor_dimension])
        """
        ...
        descriptor_dist = matches_a_descriptors - matches_b_descriptors
        match_loss = 1.0 / num_matches * (descriptor_dist).pow(2).sum()

        if norm_by_descriptor_dim:
            D = image_a_pred.shape[-1]
            match_loss /= math.sqrt(D)

        return (
            match_loss,
            matches_a_descriptors,
            matches_b_descriptors,
            torch.linalg.norm(descriptor_dist.detach().cpu().squeeze(0), dim=-1),
        )

    @staticmethod
    def non_match_descriptor_loss(
        image_a_pred,
        image_b_pred,
        non_matches_a,
        non_matches_b,
        M=0.5,
        invert=False,
        norm_by_descriptor_dim=False,
    ):
        """
        Computes the max(0, M - D(I_a,I_b,u_a,u_b))^2 term

        This is effectively:       "a and b should be AT LEAST M away from each
        other"
        With invert=True, this is: "a and b should be AT MOST  M away from each
        other"
        ...
        :type image_b_pred:
        :param non_matches_a: torch.Variable(torch.FloatTensor) has shape
        [num_non_matches,],  a (u,v) pair is mapped
        ...
        norm_degree = 2
        non_match_loss = (non_matches_a_descriptors - non_matches_b_descriptors).norm(
            norm_degree, 1
        )

        if norm_by_descriptor_dim:
            D = image_a_pred.shape[-1]
            non_match_loss /= math.sqrt(D)

        descriptor_dist = non_match_loss.detach().cpu()
        ...
        return (
            non_match_loss,
            num_hard_negatives,
            non_matches_a_descriptors,
            non_matches_b_descriptors,
            descriptor_dist,
        )

    def non_match_loss_with_l2_pixel_norm(
        self,
        image_a_pred,
        image_b_pred,
        matches_b,
        ...
        :type image_b_pred:
        :param matches_a: torch.Variable(torch.LongTensor) has shape
        [num_matches,],  a (u,v) pair is mapped
        to (u,v) ---> image_width * v + u, this matches the shape of one
        dimension of image_a_pred
        :type matches_a: torch.Variable(torch.FloatTensor)
        :param matches_b: same as matches_a
        :type matches_b:
        :param non_matches_a: torch.Variable(torch.FloatTensor) has shape
        ...
        (
            non_match_descriptor_loss,
            num_hard_negatives,
            _,
            _,
            non_match_descriptor_dist,
        ) = PCL.non_match_descriptor_loss(
            image_a_pred,
            image_b_pred,
            non_matches_a,
            non_matches_b,
            M=M_descriptor,
            norm_by_descriptor_dim=norm_by_descriptor_dim,
        )
        ...
        if self.debug:
            ...
            self._debug_data["fraction_hard_negatives"] = (
                num_hard_negatives * 1.0 / num_non_matches
            )

        return non_match_loss, num_hard_negatives, non_match_descriptor_dist

    def non_match_loss_descriptor_only(
        self,
        image_a_pred,
        image_b_pred,
        non_matches_a,
    ...
    def l2_pixel_loss(self, matches_b, non_matches_b, M_pixel=None):
        ...
        :param matches_b: A torch.LongTensor with shape
        torch.Shape([num_matches])
        :param non_matches_b: A torch.LongTensor with shape
        torch.Shape([num_non_matches])
        :return l2 loss per sample: A torch.FloatTensorof with shape
        torch.Shape([num_matches])
        """
        ...
        ground_truth_pixels_for_non_matches_b = (
            torch.t(matches_b.repeat(num_non_matches_per_match, 1))
            .contiguous()
            .view(-1, 1)
        )
        ...
        # each element is always within [0,1], you have 1 if you are at least
        # M_pixel away in L2 norm in pixel space
        norm_degree = 2
        squared_l2_pixel_loss = (
            1.0
            / M_pixel
            * torch.clamp(
                (ground_truth_u_v_b - sampled_u_v_b).float().norm(norm_degree, 1),
                max=M_pixel,
            )
        )
    ...
    def flattened_pixel_locations_to_u_v(self, flat_pixel_locations):
        """
        :param flat_pixel_locations: A torch.LongTensor of shape
        torch.Shape([n,1]) where each element is a flattened pixel index, i.e.
        some integer between 0 and 307,200 for a 640x480 image

        :type flat_pixel_locations: torch.LongTensor

        :return A torch.LongTensor of shape (n,2) where the first column is the
        u coordinates of the pixel and the second column is the v coordinate
